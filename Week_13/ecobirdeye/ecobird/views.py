...
def about(request):
    ...
    return render(request,"frontend/about.html", {'features': features})
...
def package(request):
    return render(request,"frontend/package.html")
...
def predict(request):
    if request.method == 'POST':
        ...
        image_url = f'C:/Ecobird_week_12/venv/templates/frontend/static/img/{os.path.basename(tmp_file.name)}'  # Adjust as per your media URL configuration

        # Return both the predicted label and the image URL
        # return HttpResponse(f'Predicted Label: {class_label}\nImage URL: {image_url}')
        return render(request, "frontend/result.html", {'label': class_label, 'image_url': image_url})
        # except Exception as e:
        #     return HttpResponse('Error processing the prediction', status=500)
